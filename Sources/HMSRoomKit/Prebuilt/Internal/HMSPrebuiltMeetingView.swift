...
struct HMSPrebuiltMeetingView: View {
    ...
    @State private var pollsBadgeState: HMSOptionSheetView.PollsOptionAppearance = .init(
        .none, containsItems: false)
    ...
    public var body: some View {
                ...
                case .none:
                    HMSLoadingScreen()
                        .onAppear {
                            ...
                            // If no gate is set, keep default behavior (join immediately)
                            guard let gate = HMSPrebuiltConfig.joinGateNotificationName,
                                !gate.isEmpty
                            else {
                                Task { try await roomModel.joinSession() }
                                return
                            }
                        }
                        .onReceive(
                            NotificationCenter.default.publisher(
                                for: Notification.Name(
                                    HMSPrebuiltConfig.joinGateNotificationName ?? "")
                            )
                        ) { _ in
                            // Clear gate to avoid leaking across sessions
                ...
                case .default:
                    HMSPreviewScreen { screen in
                        if let defaultScreen = roomInfoModel.defaultPreviewScreen {
                            screen = .init(
                                title: defaultScreen.elements?.preview_header?.title ?? "",
                                subTitle: defaultScreen.elements?.preview_header?.sub_title ?? "",
                                joinButtonType: defaultScreen.elements?.join_form.join_btn_type
                                    == .join ? .join : .goLive,
                                joinButtonLabel: defaultScreen.elements?.join_form.join_btn_label
                                    ?? "",
                                goLiveButtonLabel: defaultScreen.elements?.join_form
                                    .go_live_btn_label ?? "",
                                noiseCancellation: .init(
                                    startsEnabled: (defaultScreen.elements?.noise_cancellation?
                                        .enabled_by_default ?? false) ? true : false),
                                virtualBackgrounds: defaultScreen.elements?.virtual_background?
                                    .background_media?.map {
                                        HMSPreviewScreen.DefaultType.VirtualBackground(
                                            url: URL(string: $0.url)!, isDefault: $0.default,
                                            type: $0.media_type == .image ? .image : .video)
                                    } ?? [])
                        }
                    }
                    .onAppear {
        ...
        .environment(
            ...
            .constant(
                .init(
                    userStreamingState == .starting ? .compact : .full,
                    isOverlayHidden: (userStreamingState == .starting || controlsState == .hidden)))
        )
        ...
        .onAppear {
            UIPageControl.appearance().currentPageIndicatorTintColor = UIColor(
                currentTheme.colorTheme.colorForToken(.onSurfaceHigh))
            UIPageControl.appearance().pageIndicatorTintColor = UIColor(
                currentTheme.colorTheme.colorForToken(.onSurfaceLow))
        }
    }
}
